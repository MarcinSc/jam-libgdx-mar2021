...
public class FadeOutAction implements Action {
    ...
    public FadeOutAction(TimeProvider timeProvider, GraphScreenShaders graphScreenShaders,
    ...
    public boolean act() {
        ...
        if (currentTime >= start + this.time) {
            graphScreenShaders.setProperty(tag, property, 1);
            return true;
        }

        graphScreenShaders.setProperty(tag, property, (float) Math.pow((currentTime - start) / time, 2));
        return false;
    }
}
